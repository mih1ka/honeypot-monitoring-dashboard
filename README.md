...
In modern networks, cyber attacks often go undetected until serious damage occurs. Network administrators need a simple and effective way to monitor, detect, and analyze unauthorized access attempts. Traditional intrusion detection systems can be complex or expensive for small organizations and students.

Proposed Solution

This project implements a low-interaction honeypot that listens for incoming network requests and records attack data in a database. The data is then displayed on a Flask-based web dashboard with statistics, attack graphs, and logs for easy visualization and analysis.

Features

Connection Capture: Logs every incoming connection attempt with source IP, destination port, and payload.
...
Attack Analytics: Bar chart of attack counts by port and optional pie chart by country.
...
This will start listening for incoming connections and record them in the database.
...
Health Indicators: Shows database connection status and recent updates.

Future Enhancements

Integration with an external threat intelligence API for IP geolocation.

Real-time alert system via email or Telegram when a new attack is detected.

Advanced analytics dashboard with filtering and search options.

Deployment on cloud infrastructure for continuous monitoring.

Author
Mihika Manish
This project was developed as a project for my  computer networks course